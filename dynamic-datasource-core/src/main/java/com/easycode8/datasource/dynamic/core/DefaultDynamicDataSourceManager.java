...
public class DefaultDynamicDataSourceManager implements DynamicDataSourceManager, InitializingBean {
    private static final Logger LOGGER = LoggerFactory.getLogger(DefaultDynamicDataSourceManager.class);
    ...
    public void checkAllDataSource(boolean forceCheck) {
        for (Map.Entry<String, DataSource> item : dynamicDataSource.getDynamicDataSourceMap().entrySet()) {
            ...
            try {
                ...
                // 连接失败就关闭对应的数据源
                dynamicDataSource.closeDataSource(item.getKey(), dataSource);
